...
#         ax2.set_ylim(-0.5, 1.5)
        ...
        self.text.set_text('selected date: %s' % self.np.datetime_as_string(self.xs[dataind], unit='m') + ' Value: %s' % self.ys[dataind] )
    ...
    def onpan(self, p_index):
        ...
        # 50 for padding
        self.ax.set_ylim(self.np.nanmin(self.ys[left:right])-50,self.np.nanmax(self.np.ma.masked_equal(self.ys[left:right], 9999, copy=False))+50)
